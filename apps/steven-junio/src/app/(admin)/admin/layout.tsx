...
async function AdminLayout({ children }: { children: React.ReactNode }) {
  if (!(await isUserAdmin())) redirect("/auth/login?returnTo=/admin");
  return (
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-5 px-5 py-4">
          ...
          <nav className="flex flex-1 flex-wrap gap-1 text-sm text-slate-600">
            ...
            <Link href="/ask" className="rounded-lg px-3 py-2 hover:bg-slate-100 hover:text-slate-950">Public agent</Link>
          </nav>
